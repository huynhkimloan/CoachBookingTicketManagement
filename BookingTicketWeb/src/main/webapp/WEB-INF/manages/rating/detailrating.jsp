    ...
    <div class="container">
        ...
        <div>
            <c:if test="${user.active==true}">
                ...
                <a onclick="showOpenLock(${user.id})" style="cursor: pointer" class="btn btn-info">Mở khóa tài khoản</a>
                </c:if>  
            <a href="<c:url value="/admin/rating/list" />" class="btn btn-secondary">Trở lại</a>
        </div>
    </div>
</body>
...
<script>
    ...
    function showLock(id) {
        ...
        if (option === true)
            window.location.href = '/BookingTicketWeb/admin/rating/lockRatingDriver/' + id;
    }

    function showOpenLock(id) {
        var option = confirm('Bạn có chắc chắn muốn mở tài khoản này ?');
        if (option === true)
            window.location.href = '/BookingTicketWeb/admin/rating/openRatingDriver/' + id;
    }
</script>
